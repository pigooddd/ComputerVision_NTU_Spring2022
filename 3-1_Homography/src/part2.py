...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    arucoDict = aruco.Dictionary_get(aruco.DICT_4X4_50)#拿aruco.DICT_4X4_50字典
    arucoParameters = aruco.DetectorParameters_create()#初始化參數
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            corners = np.squeeze(corners[0]).astype(np.int32)
            # print(ref_corns.dtype, corners.dtype)
            ...
            #test0: 用forward warped
            # warping(ref_image, frame, H, 0, h, 0, w, direction='f')
            
            #test1: whole dst
            # h_f, w_f, c_f = frame.shape
            # warping(ref_image, frame, H, 0, h_f, 0, w_f, direction='b')
            
            #test2: part of dst ([0]是x, [1]是y)
            warping(ref_image, frame, H, np.min(corners,axis=0)[1], np.max(corners,axis=0)[1], \
