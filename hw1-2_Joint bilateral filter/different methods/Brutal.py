    ...
    def joint_bilateral_filter(self, img, guidance):
        ...
        for row in range(self.pad_w, img.shape[0]+self.pad_w):
            for column in range(self.pad_w, img.shape[1]+self.pad_w):
                ...
                center_pixel = normalized_padded_guidance[row][column]
                range_kernel = normalized_padded_guidance[row-self.pad_w:row+self.pad_w+1, column-self.pad_w:column+self.pad_w+1]
                range_kernel = -(range_kernel-center_pixel)**2/two_r_square
                if (len(normalized_padded_guidance.shape)==3):
                    range_kernel = range_kernel.sum(axis=2)
